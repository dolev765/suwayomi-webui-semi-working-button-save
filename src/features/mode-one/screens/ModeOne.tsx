...
import { useProgressiveTagFeed } from './mode-one/hooks/useProgressiveTagFeed.ts';
...
export const ModeOne = () => {
    ...
    const aggregatedFilters = useMemo(() => {
        ...
        // Override Tag Search Mode filter to only show OR and AND options
        const tagSearchModeFilter = built.find(f => f.key === 'select:tag search mode');
        if (tagSearchModeFilter && tagSearchModeFilter.type === 'select' && tagSearchModeFilter.options) {
            tagSearchModeFilter.options = [
                {
                    key: 'or',
                    label: 'OR',
                    normalizedKeys: ['or'],
                    sources: tagSearchModeFilter.options[0]?.sources || [],
                    perSourceValues: {},
                },
                {
                    key: 'and',
    ...
    // Use progressive feed for sources with tag combinations in hybrid mode
    const useProgressiveForSource = (payload: ModeOneFilterPayload) =>
        tagSearchMode === 'hybrid' && payload.tagCombinations && payload.tagCombinations.length > 1;

    // Hooks must be called unconditionally, so we call both and choose which to use
    const hentai2readProgressiveFeed = useProgressiveTagFeed(
        resolvedSources.hentai2read?.id,
        hideLibraryEntries,
        'hentai2read',
    ...
    const hentai2readRegularFeed = useSourceFeed(
        resolvedSources.hentai2read?.id,
        hideLibraryEntries,
        'hentai2read',
        filterPayloads.hentai2read,
    ...
    const hentai2readFeed = useProgressiveForSource(filterPayloads.hentai2read)
        ? {
            mangas: hentai2readProgressiveFeed.mangas,
            isLoading: hentai2readProgressiveFeed.isLoading,
            hasNextPage: hentai2readProgressiveFeed.hasNextPage,
            error: hentai2readProgressiveFeed.error,
            loadMore: hentai2readProgressiveFeed.loadMore,
            filteredOutAllItemsOfFetchedPage: hentai2readProgressiveFeed.filteredOutAllItemsOfFetchedPage,
            warnings: hentai2readProgressiveFeed.warnings,
            omittedTagsByManga: hentai2readProgressiveFeed.omittedTagsByManga,
        } as ModeOneFeedState
        : hentai2readRegularFeed;

    const hitomiProgressiveFeed = useProgressiveTagFeed(
        resolvedSources.hitomi?.id,
        hideLibraryEntries,
        'hitomi',
    ...
    const hitomiRegularFeed = useSourceFeed(
        resolvedSources.hitomi?.id,
        hideLibraryEntries,
        'hitomi',
        filterPayloads.hitomi,
    ...
    const hitomiFeed = useProgressiveForSource(filterPayloads.hitomi)
        ? {
            mangas: hitomiProgressiveFeed.mangas,
            isLoading: hitomiProgressiveFeed.isLoading,
            hasNextPage: hitomiProgressiveFeed.hasNextPage,
            error: hitomiProgressiveFeed.error,
            loadMore: hitomiProgressiveFeed.loadMore,
            filteredOutAllItemsOfFetchedPage: hitomiProgressiveFeed.filteredOutAllItemsOfFetchedPage,
            warnings: hitomiProgressiveFeed.warnings,
            omittedTagsByManga: hitomiProgressiveFeed.omittedTagsByManga,
        } as ModeOneFeedState
        : hitomiRegularFeed;

    const ehentaiProgressiveFeed = useProgressiveTagFeed(
        resolvedSources.ehentai?.id,
        hideLibraryEntries,
        'ehentai',
    ...
    const ehentaiRegularFeed = useSourceFeed(
        resolvedSources.ehentai?.id,
        hideLibraryEntries,
        'ehentai',
        filterPayloads.ehentai,
    ...
    const ehentaiFeed = useProgressiveForSource(filterPayloads.ehentai)
        ? {
            mangas: ehentaiProgressiveFeed.mangas,
            isLoading: ehentaiProgressiveFeed.isLoading,
            hasNextPage: ehentaiProgressiveFeed.hasNextPage,
            error: ehentaiProgressiveFeed.error,
            loadMore: ehentaiProgressiveFeed.loadMore,
            filteredOutAllItemsOfFetchedPage: ehentaiProgressiveFeed.filteredOutAllItemsOfFetchedPage,
            warnings: ehentaiProgressiveFeed.warnings,
            omittedTagsByManga: ehentaiProgressiveFeed.omittedTagsByManga,
        } as ModeOneFeedState
        : ehentaiRegularFeed;

    const hentaifoxProgressiveFeed = useProgressiveTagFeed(
        resolvedSources.hentaifox?.id,
        hideLibraryEntries,
        'hentaifox',
    ...
    const hentaifoxRegularFeed = useSourceFeed(
        resolvedSources.hentaifox?.id,
        hideLibraryEntries,
        'hentaifox',
        filterPayloads.hentaifox,
    ...
    const hentaifoxFeed = useProgressiveForSource(filterPayloads.hentaifox)
        ? {
            mangas: hentaifoxProgressiveFeed.mangas,
            isLoading: hentaifoxProgressiveFeed.isLoading,
            hasNextPage: hentaifoxProgressiveFeed.hasNextPage,
            error: hentaifoxProgressiveFeed.error,
            loadMore: hentaifoxProgressiveFeed.loadMore,
            filteredOutAllItemsOfFetchedPage: hentaifoxProgressiveFeed.filteredOutAllItemsOfFetchedPage,
            warnings: hentaifoxProgressiveFeed.warnings,
            omittedTagsByManga: hentaifoxProgressiveFeed.omittedTagsByManga,
        } as ModeOneFeedState
        : hentaifoxRegularFeed;

    const hentaieraProgressiveFeed = useProgressiveTagFeed(
        resolvedSources.hentaiera?.id,
        hideLibraryEntries,
        'hentaiera',
    ...
    const hentaieraRegularFeed = useSourceFeed(
        resolvedSources.hentaiera?.id,
        hideLibraryEntries,
        'hentaiera',
        filterPayloads.hentaiera,
    ...
    const hentaieraFeed = useProgressiveForSource(filterPayloads.hentaiera)
        ? {
            mangas: hentaieraProgressiveFeed.mangas,
            isLoading: hentaieraProgressiveFeed.isLoading,
            hasNextPage: hentaieraProgressiveFeed.hasNextPage,
            error: hentaieraProgressiveFeed.error,
            loadMore: hentaieraProgressiveFeed.loadMore,
            filteredOutAllItemsOfFetchedPage: hentaieraProgressiveFeed.filteredOutAllItemsOfFetchedPage,
            warnings: hentaieraProgressiveFeed.warnings,
            omittedTagsByManga: hentaieraProgressiveFeed.omittedTagsByManga,
        } as ModeOneFeedState
        : hentaieraRegularFeed;

    const imhentaiProgressiveFeed = useProgressiveTagFeed(
        resolvedSources.imhentai?.id,
        hideLibraryEntries,
        'imhentai',
    ...
    const imhentaiRegularFeed = useSourceFeed(
        resolvedSources.imhentai?.id,
        hideLibraryEntries,
        'imhentai',
        filterPayloads.imhentai,
    ...
    const imhentaiFeed = useProgressiveForSource(filterPayloads.imhentai)
        ? {
            mangas: imhentaiProgressiveFeed.mangas,
            isLoading: imhentaiProgressiveFeed.isLoading,
            hasNextPage: imhentaiProgressiveFeed.hasNextPage,
            error: imhentaiProgressiveFeed.error,
            loadMore: imhentaiProgressiveFeed.loadMore,
            filteredOutAllItemsOfFetchedPage: imhentaiProgressiveFeed.filteredOutAllItemsOfFetchedPage,
            warnings: imhentaiProgressiveFeed.warnings,
            omittedTagsByManga: imhentaiProgressiveFeed.omittedTagsByManga,
        } as ModeOneFeedState
        : imhentaiRegularFeed;

    const nhentaiProgressiveFeed = useProgressiveTagFeed(
        resolvedSources.nhentai?.id,
        hideLibraryEntries,
        'nhentai',
    ...
    const nhentaiRegularFeed = useSourceFeed(
        resolvedSources.nhentai?.id,
        hideLibraryEntries,
        'nhentai',
        filterPayloads.nhentai,
    ...
    const nhentaiFeed = useProgressiveForSource(filterPayloads.nhentai)
        ? {
            mangas: nhentaiProgressiveFeed.mangas,
            isLoading: nhentaiProgressiveFeed.isLoading,
            hasNextPage: nhentaiProgressiveFeed.hasNextPage,
            error: nhentaiProgressiveFeed.error,
            loadMore: nhentaiProgressiveFeed.loadMore,
            filteredOutAllItemsOfFetchedPage: nhentaiProgressiveFeed.filteredOutAllItemsOfFetchedPage,
            warnings: nhentaiProgressiveFeed.warnings,
            omittedTagsByManga: nhentaiProgressiveFeed.omittedTagsByManga,
        } as ModeOneFeedState
        : nhentaiRegularFeed;
            ...
            for (let offset = 0; offset < BATCH_SIZE_PER_SOURCE; offset += 1) {
                activeKeys.forEach((key) => {
                    ...
                    if (manga) {
                        ...
                        // Combine warnings from feed and omitted tags
                        const feedWarnings = feedByKey[key].warnings || [];
                        const omittedTags = feedByKey[key].omittedTagsByManga?.[manga.id] || [];
                        ...
                        if (allWarnings.length > 0) {
                            warningsByManga[manga.id] = allWarnings;
                        }
                    }
                });
            }
