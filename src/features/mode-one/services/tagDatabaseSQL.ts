...
const getInitSqlJs = async () => {
    if (!initSqlJs) {
        try {
            // Import sql.js - CommonJS module, default export is the init function
            // eslint-disable-next-line @typescript-eslint/no-require-imports
            const sqlJsModule = await import('sql.js/dist/sql-wasm.js');

            // CommonJS modules imported as ES modules expose default export
            initSqlJs = sqlJsModule.default || sqlJsModule;

            if (typeof initSqlJs !== 'function') {
                throw new Error(`sql.js init is not a function, got: ${typeof initSqlJs}`);
            }
        } catch (error) {
            dbError('sql.js import error:', error);
            throw new Error(`Failed to load sql.js: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return initSqlJs;
};
...
export const searchCustomTags = (
    ...
    try {
        // Debug: Check if database has data (only in dev, and only once per session)
        if (isDev && !(window as any).__dbDebugChecked) {
            try {
                ...
                (window as any).__dbDebugChecked = true;
            } catch (debugError) {
                // Ignore debug errors
            }
        }
        ...
        // Removed verbose debugging to improve performance
...
// Resolve alias to canonical (async version)
...
// Resolve alias to canonical (synchronous version for better performance)
export const resolveAliasSync = (alias: string): string | undefined => {
    if (!db || !dbInitialized) {
        return undefined;
    }
