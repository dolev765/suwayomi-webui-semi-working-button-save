...
export const useProgressiveTagFeed = (
    ...
    useEffect(() => {
        if (currentMangas.length > 0) {
            ...
            if (currentCombination && currentCombination.omittedTags.length > 0) {
                setAccumulatedOmittedTags(prev => {
                    const updated = { ...prev };
                    currentMangas.forEach(manga => {
                        // Only update if not already set (first combination takes priority)
                        if (!updated[manga.id]) {
    ...
    }, [tagCombinations.length > 0 ? tagCombinations[0]?.query : '']);
    ...
    useEffect(() => {
        ...
        if (isCurrentlyExhausted) {
            ...
            // Move to next combination after a short delay to ensure state is updated
            setTimeout(() => {
                const nextIndex = currentCombinationIndex + 1;
                if (nextIndex < tagCombinations.length) {
                    setCurrentCombinationIndex(nextIndex);
    ...
    return {
        ...
        hasNextPage: hasNextPage || currentCombinationIndex < tagCombinations.length - 1,
