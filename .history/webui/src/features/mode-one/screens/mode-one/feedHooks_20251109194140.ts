...
export const useSourceFeed = (
    ...
    // For OR mode, we need to handle fragments differently
    // For AND/hybrid modes, join fragments with spaces (AND behavior)
    // For OR mode, we'll make separate calls (handled below)
    const combinedQuery = useMemo(() => {
        ...
        // For OR mode, we don't combine fragments here - we'll make separate calls
        // But we still need a query for the initial call, so use the first fragment
        if (tagSearchMode === 'or' && normalizedFragments.length > 1) {
            // For OR mode, return the first fragment (we'll handle others separately)
            return base ? `${base} ${normalizedFragments[0]}` : normalizedFragments[0];
        }
        
        // For AND and hybrid modes, join all fragments with spaces
