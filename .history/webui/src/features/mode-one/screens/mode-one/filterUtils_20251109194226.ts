                        ...
                        if (isTagFilter) {
                            ...
                            if (tags.length > 1) {
                                ...
                                if (tagSearchMode === 'and') {
                                    // Strict AND: all tags together
                                    addQueryFragment(sourceKey, tags.join(' '));
                                } else if (tagSearchMode === 'or') {
                                    // OR mode: each tag separately
                                    tags.forEach(tag => addQueryFragment(sourceKey, tag));
                                } else {
                                    // Hybrid mode: AND first, then OR if needed
                                    // Add AND query (all tags together)
                                    addQueryFragment(sourceKey, tags.join(' '));
                                    // Also add individual tags for OR fallback
                                    tags.forEach(tag => addQueryFragment(sourceKey, tag));
                                }
