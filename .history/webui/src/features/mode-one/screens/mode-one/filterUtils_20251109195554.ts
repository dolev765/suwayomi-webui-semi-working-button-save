...
import { generateProgressiveTagCombinations, type TagCombination } from './progressiveTagSearch.ts';
                        ...
                        if (isTagFilter) {
                            ...
                            if (tags.length > 1) {
                                ...
                                if (tagSearchMode === 'and') {
                                    ...
                                    // Hybrid mode: search all tags together first (AND)
                                    // If not enough results, also search tags separately (OR fallback)
                                    addQueryFragment(sourceKey, tags.join(' '));
                                    // Individual tags added as fallback fragments
                                    tags.forEach(tag => addQueryFragment(sourceKey, tag));
                                }
