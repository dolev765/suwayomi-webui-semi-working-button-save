...
export const ModeOne = () => {
    ...
    // Helper to choose between progressive feed (hybrid mode with tag combinations) or regular feed
    const getFeedForSource = (
        sourceKey: ModeOneSourceKey,
        sourceId: string | undefined,
        payload: ModeOneFilterPayload,
    ) => {
        // Use progressive feed if in hybrid mode and has tag combinations
        if (tagSearchMode === 'hybrid' && payload.tagCombinations && payload.tagCombinations.length > 1) {
            const progressiveFeed = useProgressiveTagFeed(
                sourceId,
                hideLibraryEntries,
                sourceKey,
                payload,
            ...
            // Convert ProgressiveTagFeedState to ModeOneFeedState for compatibility
            return {
                mangas: progressiveFeed.mangas,
                isLoading: progressiveFeed.isLoading,
                hasNextPage: progressiveFeed.hasNextPage,
                error: progressiveFeed.error,
                loadMore: progressiveFeed.loadMore,
                filteredOutAllItemsOfFetchedPage: progressiveFeed.filteredOutAllItemsOfFetchedPage,
                warnings: progressiveFeed.warnings,
                omittedTagsByManga: progressiveFeed.omittedTagsByManga,
            } as ModeOneFeedState;
        }
        // Otherwise use regular feed
        return useSourceFeed(sourceId, hideLibraryEntries, sourceKey, payload, searchQuery);
    };

    const hentai2readFeed = getFeedForSource(
        'hentai2read',
        resolvedSources.hentai2read?.id,
        filterPayloads.hentai2read,
    );
    const hitomiFeed = getFeedForSource(
        'hitomi',
        resolvedSources.hitomi?.id,
        filterPayloads.hitomi,
    );
    const ehentaiFeed = getFeedForSource(
        'ehentai',
        resolvedSources.ehentai?.id,
        filterPayloads.ehentai,
    );
    const hentaifoxFeed = getFeedForSource(
        'hentaifox',
        resolvedSources.hentaifox?.id,
        filterPayloads.hentaifox,
    );
    const hentaieraFeed = getFeedForSource(
        'hentaiera',
        resolvedSources.hentaiera?.id,
        filterPayloads.hentaiera,
    );
    const imhentaiFeed = getFeedForSource(
        'imhentai',
        resolvedSources.imhentai?.id,
        filterPayloads.imhentai,
    );
    const nhentaiFeed = getFeedForSource(
        'nhentai',
        resolvedSources.nhentai?.id,
