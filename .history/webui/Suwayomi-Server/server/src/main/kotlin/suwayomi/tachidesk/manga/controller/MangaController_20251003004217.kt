...
object MangaController {
    ...
    @Serializable
    data class TranslatedMangaRequest(
        val originalMangaId: Int? = null,
    ...
    @Serializable
    data class TranslatedMangaResponse(
        val mangaId: Int,
    ...
    val createTranslated =
        handler(
            documentWith = {
                withOperation {
                    summary("Create translated local manga")
                    description("Persist translated images as a new local manga and add it to the library.")
                }
            },
            behaviorOf = { ctx ->
                ctx.getAttribute(Attribute.TachideskUser).requireUser()
                ...
                val payload = runCatching {
                    json.decodeFromString(TranslatedMangaRequest.serializer(), ctx.body())
                }.getOrElse {
                    ctx.status(HttpStatus.BAD_REQUEST).result("Invalid payload: ${'{'}it.message{'}'}")
                    return@handler
                }

                if (payload.images.isEmpty()) {
                    ctx.status(HttpStatus.BAD_REQUEST).result("No images provided")
                    return@handler
                }

                val now = Instant.now()
                val localDate = now.atZone(ZoneId.systemDefault()).toLocalDate()
                val timestampSuffix = DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss")
                    .withZone(ZoneId.systemDefault())
                    .format(now)

                val inferredTitle = payload.title?.takeIf { it.isNotBlank() }
                    ?: payload.originalMangaId?.let {
                        runCatching { runBlocking { Manga.getManga(it).title } }.getOrNull()
                    }
                    ?: "Translated Manga"

                val sanitizedSeriesName = sanitizeComponent("${'{'}inferredTitle.trim(){'}'} (Translated ${'{'}timestampSuffix{'}'})")
                val localRoot = File(applicationDirs.localMangaRoot).also { it.mkdirs() }
                val mangaDir = ensureUniqueDirectory(localRoot, sanitizedSeriesName)

                val chapterLabel = payload.chapterName?.takeIf { it.isNotBlank() }
                    ?: "Translation ${'{'}timestampSuffix{'}'}"
                val chapterDir = File(mangaDir, sanitizeComponent(chapterLabel)).also { it.mkdirs() }

                try {
                    var coverWritten = false
                    payload.images.forEachIndexed { index, data ->
                        val (bytes, extension) = decodeImageData(data)
                        val pageFile = File(chapterDir, "%03d.%s".format(index + 1, extension))
                        pageFile.writeBytes(bytes)
                        if (!coverWritten) {
                            File(mangaDir, "cover.${'{'}extension{'}'}").writeBytes(bytes)
                            coverWritten = true
                        }
                    }

                    writeComicInfo(
                        File(chapterDir, "ComicInfo.xml"),
                        inferredTitle.trim(),
                        chapterLabel.trim(),
                        localDate.year,
                    ...
                    writeComicInfo(
                        File(mangaDir, "ComicInfo.xml"),
                        inferredTitle.trim(),
                        "Auto-generated translated series",
                        localDate.year,
                        localDate.monthValue,
                        localDate.dayOfMonth,
                    )

                    val catalogue = suwayomi.tachidesk.manga.impl.util.source.GetCatalogueSource
                        .getCatalogueSourceOrStub(LocalSource.ID)
                    if (catalogue !is LocalSource) {
                        mangaDir.deleteRecursively()
                        ctx.status(HttpStatus.INTERNAL_SERVER_ERROR).result("Local source unavailable")
                        return@handler
                    }

                    LocalSource.pageCache.entries.removeIf { (key, _) -> key.startsWith("/${'{'}mangaDir.name{'}'}") }

                    val processed = runBlocking {
                        MangaList.run {
                            catalogue.getSearchManga(page = 1, query = mangaDir.name, filters = FilterList())
                                .processEntries(LocalSource.ID)
                        }
                    }

                    val createdManga = processed.mangaList.firstOrNull { it.title == mangaDir.name }
                        ?: processed.mangaList.firstOrNull {
                            it.title.contains(inferredTitle.trim(), ignoreCase = true)
                        }

                    if (createdManga == null) {
                        mangaDir.deleteRecursively()
                        ctx.status(HttpStatus.INTERNAL_SERVER_ERROR).result("Failed to register translated manga")
                        return@handler
                    }

                    runBlocking { Library.addMangaToLibrary(createdManga.id) }

                    ctx.json(TranslatedMangaResponse(createdManga.id, mangaDir.name))
                } catch (e: Exception) {
                    mangaDir.deleteRecursively()
                    throw e
                }
            ...
            withResults = {
                json<TranslatedMangaResponse>(HttpStatus.OK)
...
private fun sanitizeComponent(raw: String, maxLength: Int = 120): String {
    val cleaned = raw
...
private fun ensureUniqueDirectory(parent: File, desiredName: String): File {
    var candidate = File(parent, desiredName)
    var suffix = 1
    while (candidate.exists()) {
        candidate = File(parent, "${desiredName} (${suffix})")
        suffix += 1
    }
    candidate.mkdirs()
    return candidate
...
private fun decodeImageData(input: String): Pair<ByteArray, String> {
    val parts = input.split(',', limit = 2)
    val header = if (parts.size == 2) parts[0] else ""
    val dataPortion = if (parts.size == 2) parts[1] else parts[0]
    val extension = when {
    ...
    val bytes = decoder.decode(dataPortion.trim())
    return bytes to extension
}

private fun writeComicInfo(target: File, seriesTitle: String, summary: String, year: Int, month: Int, day: Int) {
    val xml = listOf(
        ...
        "  <Writer>Auto Translator</Writer>",
        "  <Publisher>Local Source</Publisher>",
        "  <Year>${year}</Year>",
        "  <Month>${month}</Month>",
        "  <Day>${day}</Day>",
