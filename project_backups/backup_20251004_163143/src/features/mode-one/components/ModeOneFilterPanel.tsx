...
export const ModeOneFilterPanel = ({
    ...
    const tagIndex = useMemo(() => {
        const map = new Map<
            string,
            { label: string; sources: Set<ModeOneSourceKey>; normalizedKeys: Set<string> }
        >();

        aggregatedFilters.forEach((filter) => {
            if (!TAG_FILTER_LABEL_PATTERN.test(filter.label)) {
                return;
            }
            filter.options?.forEach((option) => {
                ...
                MODE_ONE_QUERY_FALLBACK_SOURCES.forEach((source) => entry!.sources.add(source));
    ...
    const tagOptions = useMemo<AggregatedFilterOption[]>(() => {
        const uniqueEntries = new Set(tagIndex.values());
