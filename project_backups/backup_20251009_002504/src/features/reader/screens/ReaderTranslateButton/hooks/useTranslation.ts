        ...
        try {
            ...
            for (let i = 0; i < totalImages; i++) {
                ...
                try {
                    ...
                    const translationResponse = await fetch(`${baseUrl}/translate/with-form/image/stream/web`, {
                        method: 'POST',
                    ...
                    if (!translationResponse.ok) {
                        throw new Error(
                            `Translation API error: ${translationResponse.status} ${translationResponse.statusText}`,
                        );
                    }

                    // Decode streaming response
                    const responseBlob = enableNonStreamFallback
                        ? await translationResponse.blob()
                        : await decodeTranslatorStreamToBlob(translationResponse);
