...
import { getDoublePageModePages, createPagesData } from '@/features/reader/viewer/pager/ReaderPager.utils.tsx';
...
> &
    ...
    useEffect(() => {
        ...
        if (pagesOverride && pagesOverride.length > 0) {
            const overridePages = createPagesData(pagesOverride);
            overrideAppliedRef.current = true;

            setArePagesFetched(true);
            setContextTotalPages(pagesOverride.length);
            setTotalPages(pagesOverride.length);
            setContextPages(overridePages);
            setPages(overridePages);

            const loadStates = overridePages.map(({ primary: { url } }) => ({ url, loaded: false }));
            setContextPageLoadStates(loadStates);
            setPageLoadStates(loadStates);
            setPagesToSpreadState(overridePages.map(({ primary: { url } }) => ({ url, isSpread: false })));
            setPageUrls(pagesOverride);

            setContextCurrentPageIndex((prev) => {
                const maxIndex = Math.max(0, pagesOverride.length - 1);
                return Math.min(Math.max(prev, 0), maxIndex);
            });
            setPageToScrollToIndex((prev) => {
                const maxIndex = Math.max(0, pagesOverride.length - 1);
                return typeof prev === 'number' ? Math.min(Math.max(prev, 0), maxIndex) : 0;
            });
            setTransitionPageMode(ReaderTransitionPageMode.NONE);
            return;
        }

        if (!pagesOverride && overrideAppliedRef.current) {
        ...
        isCurrentChapter,
        setArePagesFetched,
        setContextTotalPages,
        setTotalPages,
        setContextPages,
        setPages,
        setContextPageLoadStates,
        setPageLoadStates,
        setPagesToSpreadState,
        setPageUrls,
        setContextCurrentPageIndex,
        setPageToScrollToIndex,
        setTransitionPageMode,
        doFetchPages,
    ]);
