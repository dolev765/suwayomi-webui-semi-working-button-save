...
const SectionPaper = styled(Paper)(({ theme }) => ({
    ...
    '&:hover': {
        backgroundColor: '#222',
        border: `1px solid ${alpha('#ea4c89', 0.25)}`,
    },
}));
...
const ActiveFiltersContainer = styled(Box)(({ theme }) => ({
    ...
    boxShadow: `0 0 10px ${alpha('#ea4c89', 0.1)}`,
}));

const TagSearchContainer = styled(Box)(({ theme }) => ({
    padding: theme.spacing(2.5),
    borderRadius: theme.spacing(0.5),
    background: `linear-gradient(135deg, ${alpha('#ea4c89', 0.12)} 0%, ${alpha('#c369ff', 0.08)} 100%)`,
    border: `1px solid ${alpha('#ea4c89', 0.25)}`,
    backgroundColor: '#1a1a1a',
    boxShadow: `0 0 15px ${alpha('#ea4c89', 0.15)}`,
}));
...
const SelectFilterControl = ({
    ...
    return (
        <Stack spacing={1.5}>
                    ...
                    <Select
                        ...
                        sx={{
                            ...
                            '&:hover .MuiOutlinedInput-notchedOutline': {
                                borderColor: alpha(supportColor, 0.5),
                            },
                        ...
                        MenuProps={{
                            PaperProps: {
                                sx: {
                                    backgroundColor: '#1a1a1a',
                                    border: `1px solid ${alpha(supportColor, 0.3)}`,
                                    '& .MuiMenuItem-root': {
                                        color: '#fff',
                                        '&:hover': {
                                        ...
                                        '&.Mui-selected': {
                                            backgroundColor: alpha(supportColor, 0.25),
                                            '&:hover': {
                                                backgroundColor: alpha(supportColor, 0.35),
                                            },
                                        },
                                    },
                                },
            ...
            {!!hintText && (
                <Typography variant="caption" sx={{ px: 0.5, color: alpha('#fff', 0.5) }}>
                    {hintText}
                </Typography>
            )}
        </Stack>
                    ...
                    InputProps={{
                        endAdornment: (
                            <InputAdornment position="end">
                                <SearchIcon fontSize="small" sx={{ color: supportColor }} />
                            </InputAdornment>
                        ),
                    }}
                    variant="standard"
...
export const ModeOneFilterPanel = ({
    ...
    const tagSearchMatch = useMemo(() => {
        ...
        // Try partial match in the tag index keys
        const indexKeys = Array.from(tagIndex.keys());
        const exactPartialMatch = indexKeys.find(key => key === normalized || key.includes(normalized) || normalized.includes(key));
        if (exactPartialMatch) {
            entry = tagIndex.get(exactPartialMatch);
            if (entry) {
                return tagOptionLookup.get(entry);
            }
        }
        ...
        >
            <DialogTitle sx={{
                ...
                <Stack direction="row" alignItems="center" spacing={1}>
                    <FilterAltIcon sx={{ color: '#ea4c89' }} />
                    <Typography variant="h6" sx={{ color: '#ea4c89', fontWeight: 700 }}>
            ...
            <DialogContent dividers sx={{
                ...
                <SectionPaper elevation={0}>
                    <Stack spacing={2}>
                        <Stack direction="row" alignItems="center" spacing={1}>
                            <SearchIcon sx={{ color: '#ea4c89' }} fontSize="small" />
                            <Typography variant="subtitle2" fontWeight={700} sx={{ color: '#ea4c89' }}>
                        ...
                        <TextField
                            ...
                            InputProps={{
                                endAdornment: (
                                    <InputAdornment position="end">
                                        <SearchIcon fontSize="small" sx={{ color: '#ea4c89' }} />
                                    </InputAdornment>
                                ),
                            }}
                            autoComplete="off"
                        ...
                        <Typography
                            variant="caption"
                            sx={{
                                color: !liveUpdatesEnabled && hasPendingChanges ? '#ff9800' : alpha('#fff', 0.6),
                                px: 0.5,
                            }}
                        >
                            {liveUpdatesEnabled
                ...
                <TagSearchContainer>
                    <Stack spacing={2}>
                        <Stack direction="row" alignItems="center" spacing={1}>
                            <LocalOfferIcon sx={{ color: '#ea4c89' }} fontSize="small" />
                            <Typography variant="subtitle2" fontWeight={700} sx={{ color: '#ea4c89' }}>
                            ...
                            >
                                ...
                                <TextField
                                    ...
                                    onBlur={(event) => {
                                        ...
                                        // First check if there's already a match (direct or synonym)
                                        if (tagSearchMatch) {
                                            setTagSearchValue(tagSearchMatch.label);
                                            return;
                                        }
                                        
                                        // Only auto-correct if the Levenshtein distance is very small (1-2 chars)
                                        // to avoid incorrect corrections like "titjob" -> "handjob"
                                        const closest = findClosestOption(value, tagOptions);
                                        if (closest) {
                                            const candidates = closest.normalizedKeys?.length 
                                                ? closest.normalizedKeys 
                                                : [closest.label.toLowerCase()];
                                            
                                            const hasVeryCloseMatch = candidates.some((candidate) => {
                                                const distance = levenshteinDistance(value, candidate);
                                                return distance <= 2; // Only auto-correct if 1-2 character difference
                                            });
                                    ...
                                    InputProps={{
                                        endAdornment: (
                                            <InputAdornment position="end">
                                                <SearchIcon fontSize="small" sx={{ color: tagSupportColor }} />
                                            </InputAdornment>
                                        ),
                                    }}
                                    variant="standard"
                                    ...
                                    sx={{
                                        ...
                                        '& .MuiInput-root:before': {
                                            borderBottomColor: alpha(tagSupportColor, 0.3),
                                        },
                                        '& .MuiInput-root:hover:not(.Mui-disabled):before': {
                                            borderBottomColor: alpha(tagSupportColor, 0.5),
                                        },
                                        '& .MuiInput-root.Mui-focused:after': {
                                            borderBottomColor: tagSupportColor,
                                        },
                                        '& .MuiInputBase-input::placeholder': {
                                            color: alpha('#fff', 0.5),
                                            opacity: 1,
                        ...
                        <Typography
                            variant="caption"
                            sx={{
                                color: tagSearchMatch ? '#66bb6a' : alpha('#fff', 0.6),
                                px: 0.5,
                    ...
                    <ActiveFiltersContainer>
                        <Stack spacing={1.5}>
                            <Stack direction="row" alignItems="center" justifyContent="space-between">
                                <Stack direction="row" alignItems="center" spacing={1}>
                                    <FilterAltIcon sx={{ color: '#ea4c89' }} fontSize="small" />
                                    <Typography variant="subtitle2" fontWeight={700} sx={{ color: '#ea4c89' }}>
                    ...
                    <SectionPaper elevation={0}>
                        <Stack spacing={2}>
                            <Stack spacing={0.5}>
                                ...
                                <Typography variant="caption" sx={{ pl: 3.5, color: alpha('#fff', 0.6) }}>
                                    ...
                                    {commonFilters.map((filter, index) => {
                                        ...
                                        return (
                                            <Stack key={filter.key} spacing={1}>
                                                <Typography variant="subtitle1" sx={{ fontWeight: 700, fontSize: '0.95rem', color: '#fff' }}>
                                                    {filter.label}
                                                </Typography>
                                                {control}
                                                {index < commonFilters.length - 1 && (
                                                    <Divider sx={{ opacity: 0.15, mt: 1, borderColor: alpha('#ea4c89', 0.2) }} />
                                                )}
                                            </Stack>
                                        );
                                    })}
                    ...
                    <SectionPaper elevation={0}>
                        <Stack spacing={2}>
                            <Stack spacing={0.5}>
                                ...
                                >
                                    <Stack direction="row" alignItems="center" spacing={1}>
                                        <FilterAltIcon sx={{ color: alpha('#ea4c89', 0.7) }} fontSize="small" />
                                        <Typography variant="subtitle2" fontWeight={600} sx={{ color: alpha('#fff', 0.8) }}>
                                            Advanced Filters
                                        </Typography>
                                    </Stack>
                                    <ExpandButton
                                        ...
                                        sx={{ color: alpha('#ea4c89', 0.7) }}
                                    >
                                        <ExpandMoreIcon />
                                    </ExpandButton>
                                </Stack>
                                <Typography variant="caption" sx={{ pl: 3.5, color: alpha('#fff', 0.5) }}>
