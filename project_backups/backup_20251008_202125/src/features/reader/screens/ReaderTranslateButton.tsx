...
const ReaderTranslateButton: React.FC<ReaderTranslateButtonProps> = ({ mangaId, chapterId }) => {
  ...
  const handleTranslate = async () => {
    ...
    try {
        ...
        try {
          ...
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              ...
              console.warn(`Image ${imageNumber}: Fetch attempt ${attempt} failed:`, fetchError);
              if (attempt === maxRetries) {
                throw new Error(`Failed to fetch image after ${maxRetries} attempts: ${(fetchError as Error).message}`);
              }
          ...
          // Debug image blob metadata before conversion
          console.log(`Image ${imageNumber}: Source blob info`, {
            size: imageBlob.size,
            type: imageBlob.type,
          ...
          if (contentType.includes('application/zip')) {
            throw new Error('Received ZIP archive instead of image; enable streaming endpoints or disable ZIP responses on the translation server.');
          }
      ...
      if (successCount > 0) {
        ...
        const completionMessage = `Translation completed. ${successCount} image${successCount === 1 ? '' : 's'} translated${
  ...
  const applyPreviewToReader = useCallback(() => {
    ...
    setDownloadProgress('Applied translation preview to current reader tab.');
    setError(null);
  }, [
    translationResult,
    updatePageUrls,
            ...
            <Box sx={{ mt: 2, p: 2, bgcolor: 'success.light', borderRadius: 1 }}>
              ...
              <Typography variant="body2" color="success.dark">
                ...
                {translationResult.failureCount
                  ? `, ${translationResult.failureCount} failed`
                  : ''}.
              </Typography>
              <Typography variant="caption" color="success.dark" display="block">
                Use the preview buttons below to apply the translation in this tab or launch the official reader in a new
                window.
              </Typography>
            </Box>
          )}
        ...
        <DialogActions sx={{ flexWrap: 'wrap', gap: 1, justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            <Button
              ...
              disabled={
                loading ||
                !(translationResult && translationResult.translatedPages.length > 0)
              }
            >
              Preview in Reader
            </Button>
            <Button
              variant="outlined"
              onClick={handleOpenOfficialReader}
              disabled={
                loading ||
                !(translationResult && translationResult.translatedPages.length > 0)
              }
