            ...
            descriptors.forEach((descriptor) => {
                ...
                if (!existing) {
                    ...
                    if (descriptor.type === 'select') {
                        descriptor.values.forEach((value) => {
                            ...
                            newEntry.options?.push({ key: value, label: value, sources: [sourceKey] });
                        });
                    }
                ...
                if (descriptor.type === 'select' && existing.options) {
                    descriptor.values.forEach((value) => {
                        ...
                        const option = existing.options?.find((candidate) => candidate.key === value);
                        if (option) {
                            if (!option.sources.includes(sourceKey)) {
                                option.sources.push(sourceKey);
                            }
                        } else {
                            existing.options?.push({ key: value, label: value, sources: [sourceKey] });
                        }
                    });
                }
            });
