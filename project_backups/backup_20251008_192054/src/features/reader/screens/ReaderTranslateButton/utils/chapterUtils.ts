...
export const downloadChapter = async (
    ...
    try {
        ...
        const pageUrls: string[] = pagesResult.data.fetchChapterPages?.pages?.map((p: any) => p.url) || [];
        console.log(`Retrieved ${pageUrls.length} page URL(s) for chapter ${internalChapterId}`);
